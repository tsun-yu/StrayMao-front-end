  ...
  return (
    <>
      <Navbar
        ...
        // display = "flex"
      >
        <Navbar.Brand href="/homeTest" className="ml-5"style={{color:"#FFF1E6",fontFamily:"Roboto ",fontSize: "60px"}}>Stray Mao</Navbar.Brand>
        <Navbar.Brand href="/homeTest" className="navLogoCN" style={{color:"#FFF1E6",fontSize: "16px"}}>寵愛有家</Navbar.Brand>
        <Navbar.Toggle aria-controls="responsive-navbar-nav" />
        <Navbar.Collapse id="responsive-navbar-nav">
          <Nav className="mx-auto">
            {/* 把Nav.Link作為NavLink來使用 */}
            {/* 一定要加上exact，不然首頁會一直點亮(active) */}
            <Nav.Link as={NavLink} to="/adoptionTest" style={{color:"#FFF1E6"}}>
              領養<i className="fas fa-caret-down"></i>
            </Nav.Link>
            <Nav.Link as={NavLink} to="/storeTest" style={{color:"#FFF1E6"}}>
             商城<i className="fas fa-caret-down"></i>
            </Nav.Link>
            <Nav.Link as={NavLink} to="/socialTest" style={{color:"#FFF1E6"}}>
